...
import com.apps.fast.launch.launchviews.controls.ABMSystemControl;
...
public class ABMSiteView extends StructureView
{
    ...
    public ABMSiteView(LaunchClientGame game, MainActivity activity, LaunchEntity structure)
    {
    ...
    {
        systemView = new ABMSystemControl(game, activity, structureShadow.GetID());
            ...
            public void run()
            {
                ...
                if(structure != null)
                {
                    if(game.EntityIsFriendly(structure, game.GetOurPlayer()) && structureShadow.GetOwnerID() == game.GetOurPlayerID() && !structure.GetSelling() && game.GetOurPlayer().Functioning())
                    {
                            ...
                            txtEngageDistanceButton.setText(String.format("%s (Tap to edit)", TextUtilities.GetDistanceStringFromKM(samSite.GetEngagementSpeed())));
                            lytMode.setVisibility(VISIBLE);
                        }
                        else
                        {
    ...
    {
        game.SetStructureOnOff(structureShadow.GetID(), EntityType.ABM_SILO, bOnline);
    }
}
