...
public class Utilities
{
    ...
    public static boolean DeviceHasValidID(Context context)
    {
        ...
        try
        {
            String strDeviceID = UUID.randomUUID().toString().replace("-", "");
    ...
    public static String GetEntityName(Context context, NamableInterface namable)
    {
        if(namable != null)
        {
            if(namable.GetName().length() > 0)
            {
                return namable.GetName();
            }
            else
            {
        ...
        return "[NAMABLE NULL! TELL THE DEVS.]";
    }
