 ...
 * - Primary rule: if seconds < minSeconds AND distance >= jumpMeters ⇒ HIGH score.
 * - Secondary rule: if average speed is absurd (e.g., > maxAvgMps), score too.
 */
public final class TeleportDetector {
    private final double jumpMeters;  
    private final double minSeconds;  
    private final double maxAvgMps;  
    ...
    public ScoreEvent check(long nowMs, int pid, GeoCoord prev, GeoCoord curr, double seconds) {
        ...
        if (seconds < minSeconds && meters >= jumpMeters) {
            ...
            double score = 25.0 + Math.min(50.0, over * 60.0);
        ...
        if (seconds > 0.0) {
            ...
            if (mps > maxAvgMps) {
                ...
                double score = 18.0 + Math.min(40.0, over * 50.0); 
