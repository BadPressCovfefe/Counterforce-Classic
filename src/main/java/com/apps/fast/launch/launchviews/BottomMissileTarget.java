...
public class BottomMissileTarget extends LaunchView
{
    ...
    protected void Setup()
    {
        ...
        btnFire.setOnClickListener(new OnClickListener()
        {
            ...
            public void onClick(View view)
            {
                ...
                else if((bFromPlayer && game.GetOurPlayer().GetPosition().DistanceTo(geoTarget) > game.GetConfig().GetMissileRange(missileType)) ||
                        (!bFromPlayer && game.GetMissileSite(lSiteID).GetPosition().DistanceTo(geoTarget) > game.GetConfig().GetMissileRange(missileType)))
                {
                    //Target out of range.
    ...
    private void SecondStateFiringChecks()
    {
        ...
        if(AffectedPlayers.size() == 1)
        {
            ...
            else if(lLastWarnedID != affectedPlayer.GetID() && affectedPlayer.GetNewb())
            {
                final LaunchDialog launchDialog = new LaunchDialog();
                launchDialog.SetHeaderLaunch();
        ...
        activity.runOnUiThread(new Runnable()
        {
            ...
            public void run()
            {
                ...
                else
                {
                    ...
                    txtFlightTime.setText(context.getString(R.string.flight_time_target, TextUtilities.GetTimeAmount(game.GetTimeToTarget(geoFrom, geoTarget, game.GetConfig().GetMissileSpeed(missileType)))));
