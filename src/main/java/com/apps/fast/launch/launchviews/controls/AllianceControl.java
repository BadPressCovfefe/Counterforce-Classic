...
public class AllianceControl extends LaunchView
{
    ...
    private LinearLayout btnBreakAffiliation;
    ...
    protected void Setup()
    {
        ...
        btnBreakAffiliation = findViewById(R.id.btnBreakAffiliation);
        ...
        btnBreakAffiliation.setOnClickListener(new OnClickListener()
        {
            @Override
            public void onClick(View view)
            {
                final LaunchDialog launchDialog = new LaunchDialog();
                launchDialog.SetHeaderDiplomacy();
                launchDialog.SetMessage(context.getString(R.string.confirm_break_affiliation, allianceShadow.GetName()));
                launchDialog.SetOnClickYes(new View.OnClickListener()
                {
                    @Override
                    public void onClick(View view)
                    {
                        launchDialog.dismiss();
                        game.BreakAffiliation(allianceShadow.GetID());
                    }
                });
                launchDialog.SetOnClickNo(new View.OnClickListener()
                {
        ...
        activity.runOnUiThread(new Runnable()
        {
            ...
            public void run()
            {
                ...
                if(alliance != null)
                {
                    ...
                    boolean bShowBreakAffiliation = false;
                    ...
                    if (ourPlayer.GetIsAnMP() && ourPlayer.GetAllianceMemberID() != alliance.GetID())
                    {
                        ...
                        if(game.GetAllegiance(ourPlayer, alliance) == LaunchGame.Allegiance.ENEMY)
                            bShowSurrenderWar = true;

                        if(game.GetAllegiance(ourPlayer, alliance) == LaunchGame.Allegiance.AFFILIATE)
                            bShowBreakAffiliation = true;
                    }
                    ...
                    btnBreakAffiliation.setVisibility(bShowBreakAffiliation ? VISIBLE : GONE);
