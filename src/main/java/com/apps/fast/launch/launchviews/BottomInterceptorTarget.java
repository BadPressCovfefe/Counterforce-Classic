        ...
        btnFire.setOnClickListener(new OnClickListener()
        {
            ...
            public void onClick(View view)
            {
                ...
                else
                {
                    ...
                    if (game.GetConfig().GetInterceptorSpeed(interceptorType) <= game.GetConfig().GetMissileSpeed(missileType))
                    {
        ...
        activity.runOnUiThread(new Runnable()
        {
            ...
            public void run()
            {
                ...
                else
                {
                    ...
                    GeoCoord geoFrom = bFromPlayer ? game.GetOurPlayer().GetPosition() : game.GetSAMSite(lSiteID).GetPosition();
                    GeoCoord geoIntercept = target.GetPosition().InterceptPoint(geoMissileTarget, game.GetConfig().GetMissileSpeed(missileType), geoFrom, game.GetConfig().GetInterceptorSpeed(interceptorType));
                    long oInterceptTime = game.GetTimeToTarget(geoFrom, geoIntercept, game.GetConfig().GetInterceptorSpeed(interceptorType));
                    float fltDistance = geoFrom.DistanceTo(target.GetPosition());
