...
public class StructureMaintenanceView extends LaunchView implements LaunchUICommon.StructureOnOffInfoProvider
{
    ...
    protected void Setup()
    {
        ...
        if(iconControlStructure instanceof MissileSite)
        {
            ...
            {
                imgType.setImageResource(R.drawable.icon_missilesitenuke);
        ...
        else if(iconControlStructure instanceof SAMSite)
        {
            ...
            {
                imgType.setImageResource(R.drawable.icon_abmsite_offline);
            }
            else
            {
        ...
        {
            imgType.setImageResource(R.drawable.icon_radarstation);
        }
        else if(iconControlStructure instanceof SolarPanel)
        {
            imgType.setImageResource(R.drawable.icon_solarpanel);
        }
        else if(iconControlStructure instanceof Bunker)
        {
            imgType.setImageResource(R.drawable.icon_bunker);
        }
    ...
    public void Update()
    {
        if(structureShadow != null)
        {
            ...
            if (structure != null)
            {
                ...
                {
                    txtCost.setText(TextUtilities.GetCurrencyString(5)); //Used to be 0. Changed to 10% of online cost as detailed in LaunchGame. -Corbin
                    txtTime.setVisibility(GONE);
                }
                else
                {
    ...
    public Structure GetCurrentStructure()
    {
        ...
        else if(structureShadow instanceof RadarStation)
        {
            return game.GetRadarStation(structureShadow.GetID());
        }
        else if(structureShadow instanceof SolarPanel)
        {
            return game.GetSolarPanel(structureShadow.GetID());
        }
        else if(structureShadow instanceof Bunker)
        {
    ...
    public List<Structure> GetCurrentStructures()
    {
        ...
        for(Object object : StructureList)
        {
            ...
            else if(controlStructure instanceof RadarStation)
            {
                CurrentStructures.add(game.GetRadarStation(structure.GetID()));
            }
            else if(controlStructure instanceof SolarPanel)
            {
                CurrentStructures.add(game.GetSolarPanel(structure.GetID()));
            }
            else if(controlStructure instanceof Bunker)
            {
                CurrentStructures.add(game.GetBunker(structure.GetID()));
            }
        }
    ...
    public void SetOnOff(boolean bOnline)
    {
        if(structureShadow != null)
        {
            ...
            {
                game.SetRadarStationOnOff(structureShadow.GetID(), bOnline);
            }
            else if (structureShadow instanceof SolarPanel)
            {
        ...
        else
        {
            ...
            {
                game.SetRadarStationsOnOff(IDs, bOnline);
            }
            else if(controlStructure instanceof SolarPanel)
            {
                game.SetSolarPanelsOnOff(IDs, bOnline);
            }
            else if(controlStructure instanceof Bunker)
            {
                game.SetBunkersOnOff(IDs, bOnline);
            }
        }
    }
