...
public class RadarStationRenderer extends DefaultClusterRenderer<LaunchClusterItem>
{
    private LaunchClientGame game;
    private Context context;
    private GoogleMap map;

    public RadarStationRenderer(Context context, LaunchClientGame game, GoogleMap map, ClusterManager clusterManager)
    {
    ...
    {
        RadarStation radarStation = game.GetRadarStation(item.GetID());

        if(radarStation != null && game != null)
        {
            //markerOptions.icon(BitmapDescriptorFactory.fromBitmap(StructureIconBitmaps.GetStructureBitmap(context, game, radarStation)));
            markerOptions.anchor(0.5f, 0.5f);
            markerOptions.alpha(radarStation.GetRespawnProtected() ? 0.5f : 1.0f);
        }
    }
    ...
    {
        markerOptions.icon(BitmapDescriptorFactory.fromResource(R.drawable.cluster_radarstation));
    }
