...
public class SelectInterceptorView extends LaunchView
{
    ...
    public void Update()
    {
        ...
        //Removed due to performance issues. TODO: Respond only to changes that affect the entries.
        //RebuildList()
    }
                    ...
                    if(type.GetABM() && missileType.GetICBM() || !type.GetABM() && !missileType.GetICBM())
                    {
                        ...
                        if (geoFrom.DistanceTo(missile.GetPosition()) <= game.GetConfig().GetInterceptorRange(type))
                        {
                            ...
                            if (oTimeToIntercept < game.GetTimeToTarget(missile) && !game.GetInterceptorTooSlow(type.GetID(), missileType.GetID()))
                            {
                                ...
                                final LaunchableSelectionView interceptorSelectionView = new LaunchableSelectionView(activity, game, type, oTimeToIntercept, lSAMSiteID);
                                ...
                                for (byte cSlot = 0; cSlot < system.GetSlotCount(); cSlot++)
                                {
                                    if (system.GetSlotReadyToFire(cSlot))
                                        if (system.GetSlotMissileType(cSlot) == lType)
                                            lNumber++;
                                }
