...
public final class NetClassifier {
    public enum IpKind { UNKNOWN, HOME_LIKELY, MOBILE_LIKELY, VPN_LIKELY }
    ...
    public static final class IpProfile {
        ...
        public IpProfile(String ip, long ts) {
            this.ip = ip; this.firstSeenMs = ts; this.lastSeenMs = ts; this.seenCount = 1;
        }
        public long stabilityMs() { return lastSeenMs - firstSeenMs; }
    }
    ...
    public IpProfile note(int playerId, String ip, long nowMs) {
        ...
        if (p.stabilityMs() > 86_400_000L && p.seenCount >= 6) p.kind = IpKind.HOME_LIKELY;
        return p;
    }

    public IpKind classify(int playerId, String currentIp) {
        Map<String, IpProfile> map = store.get(playerId);
        if (map == null) return IpKind.UNKNOWN;
