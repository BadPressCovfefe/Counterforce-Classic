...
public class MissileSystem extends LaunchSystem
{
    ...
    /** New. */
    public MissileSystem(LaunchSystemListener listener, int lReloadTime, int lMissileSlotCount)
    {
        super(listener);
    ...
    /** From save. */
    public MissileSystem(int lReloadRemaining, int lReloadTime, int lMissileSlotCount, Map<Integer, Integer> SlotTypes, Map<Integer, ShortDelay> PrepTimes)
    {
        super();
    ...
    public void SetReloadTimeRemaining(int lReloadTime)
    {
        this.dlyReload.Set(lReloadTime);
    }
    ...
    public void Fire(int lSlot, int lReloadTime)
    {
        MissileSlotTypes.remove(lSlot);
        MissileSlotPrepTimes.remove(lSlot);
        dlyReload.Set(lReloadTime);
    ...
    public int GetReadySlotCount()
    {
        ...
        for(ShortDelay delay : MissileSlotPrepTimes.values())
        {
            if(delay.Expired())
                lCount++;
        }
    ...
    public Map<Integer, Integer> GetReadyTypeCounts()
    {
        Map<Integer, Integer> Result = new HashMap<>();
        ...
        for(Entry<Integer, Integer> MissileSlotType : MissileSlotTypes.entrySet())
        {
            ...
            if(lKey != MISSILE_SLOT_EMPTY_TYPE && GetSlotReadyToFire(MissileSlotType.getKey()))
            {
                if(Result.containsKey(lKey))
                {
                    int lNewValue = Result.get(lKey) + 1;
