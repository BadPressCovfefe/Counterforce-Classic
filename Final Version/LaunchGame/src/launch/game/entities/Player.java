 ...
 * @author tobster
 */
public final class Player extends MapEntity implements LaunchSystemListener, HaulerInterface
{
    ...
    public Player(int lID, String strName, int lAvatarID, int lWealth)
    {
        ...
        this.resources = new CargoSystem(this, Long.MAX_VALUE);
        AddWealth(lWealth);
    ...
    public Player(ByteBuffer bb, int lReceivingID)
    {
        ...
        resources = new CargoSystem(this, bb);
    ...
    public byte[] GetData(int lAskingID)
    {
        ...
        byte[] cResourcesData = resources.GetData(lAskingID);
        byte[] cBlacklistData = LaunchUtilities.GetIntListData(Blacklist);
        
        ByteBuffer bb = ByteBuffer.allocate(cBaseData.length + DATA_SIZE + LaunchUtilities.GetStringDataSize(strName) + cResourcesData.length + cBlacklistData.length);
    ...
    { 
        return resources.GetAmountOfType(ResourceType.WEALTH); 
    }
    ...
    {
        return resources.ChargeQuantity(ResourceType.WEALTH, lWealth);
    }
    
    public void AddWealth(long oWealth)
    {
        resources.AddResource(ResourceType.WEALTH, oWealth);
        Changed(false);
    }
