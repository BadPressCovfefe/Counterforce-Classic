...
public class LaunchServerGame extends LaunchGame implements LaunchServerGameInterface
{
    ...
    private void ProcessHourlyMaintenance()
    {
        for(Player player : GetPlayers())
        {
            ...
            player.GetCargoSystem().ChargeQuantity(ResourceType.WEALTH, (long)lTotalCost);
        }
    }
                                                                ...
                                                                if(interceptorType.GetABM() && missileType.GetICBM() || !interceptorType.GetABM() && !missileType.GetICBM())
                                                                {
                                                                    ...
                                                                    if(fltInterceptorSpeed > fltMissileSpeed)
                                                                    {
                                                                        if(candidateType != null)
                                                                        {
                                                                            ...
                                                                            if(GetTotalResourceCost(interceptorType) < GetTotalResourceCost(candidateType))
                                                                            {
                                                                            ...
                                                                            else if(GetTotalResourceCost(interceptorType) == GetTotalResourceCost(candidateType))
                                                                            {
                    ...
                    if(!missileType.GetStealth() || fltSAMToMissileDistance <= Defs.STEALTH_ENGAGEMENT_DISTANCE)
                    {
                        if(IsInterceptorValidForMissile(interceptorType, missileType, missile))
                        {
                            if(fltSAMToMissileDistance < config.GetInterceptorRange(interceptorType))
                            {
                                ...
                                if(lTimeToIntercept < lMissileTimeToTarget)
                                {
                                    if(candidateType == null || GetTotalResourceCost(interceptorType) < GetTotalResourceCost(candidateType) || (GetTotalResourceCost(interceptorType) == GetTotalResourceCost(candidateType) && lTimeToIntercept < lCandidateTimeToIntercept))
                                    {
                                        candidateType = interceptorType;
                                        lCandidateTimeToIntercept = lTimeToIntercept;
            ...
            for(Loot loot : GetLoots())
            {
                if(loot.GetLootType() == LootType.RESOURCES && geoPlayer.BroadPhaseCollisionTest(loot.GetPosition()))
                {
                    if(geoPlayer.DistanceTo(loot.GetPosition()) <= config.GetOreCollectRadius())
                    {
                        if(!loot.Collected())
                        {
                            loot.SetQuantity(player.GetCargoSystem().AddResource(ResourceType.values()[loot.GetCargoID()], loot.GetQuantity()));
                        }
                    }
                }
            }
    ...
    private boolean ValidateConstructionRequest(Player player, GeoCoord geoCheck, String structureName, Map<ResourceType, Long> Costs)
    {
        ...
        if(player.Functioning() && geoCheck != null)
        {
            ...
            if(Result.isEmpty())
            {
                if(player.GetCargoSystem().ChargeQuantities(Costs))
                {
                    return true;
                }
                else
    ...
    private boolean ProcessPlayerPurchase(int lPlayerID, Map<ResourceType, Long> costs, ResourceSystem resources, CargoSystem cargo, boolean bUseSubstitutes, PurchaseType purchaseType)
    {
        ...
        if(player != null)
        {
            ...
            if(player.Functioning())
            {
                ...
                else if(player.GetCargoSystem().ChargeQuantities(GetRequiredCost(costs)))
                {
                    return true;
                }
                else
    ...
    private boolean ProcessPlayerWeaponPurchase(int lPlayerID, Map<ResourceType, Long> costs, ResourceSystem resources, CargoSystem cargo, boolean bOffensive)
    {
        ...
        if(player != null)
        {
            ...
            if(player.Functioning())
            {
                if(player.GetCargoSystem().ChargeQuantities(costs))
                {
                    long oSpending = costs.get(ResourceType.WEALTH);

                    if(costs.containsKey(ResourceType.ENRICHED_URANIUM))
                    {
                        oSpending += Defs.FISSILE_SCRAP_VALUE_KG * costs.get(ResourceType.ENRICHED_URANIUM);
                    }
                    
                    if(bOffensive)
                    {
                        Scoring_OffenceSpending(player, oSpending);
                    }
                    else
                    {
                        Scoring_DefenceSpending(player, oSpending);
                    }
        ...
        if(player != null && aircraft != null && targetEntity != null)
        {
            if(aircraft.EWReady() && ElectronicWarfareTargetValid(targetEntity) && !WouldBeFriendlyFire(GetOwner(targetEntity), player) && targetEntity.GetPosition().DistanceTo(aircraft.GetPosition()) <= Defs.ELECTRONIC_WARFARE_RANGE)
            {
                ...
                if(targetEntity instanceof Structure)
                {
                    ...
                    {
                        CargoSystem system = structureOwner.GetCargoSystem();
                        
                        //TODO: Record for player.
                        if(system.ChargeQuantity(ResourceType.ELECTRONICS, Defs.EMP_RESIST_ELECTRONICS_COST))
                        {
                            CreateReport(structureOwner, new LaunchReport(String.format("Your %s used %d electronics to resist %s's electronic warfare!", structure.GetTypeName(), Defs.EMP_RESIST_ELECTRONICS_COST, player.GetName()), true, player.GetID(), structureOwner.GetID()));
                            CreateReport(player, new LaunchReport(String.format("%s's %s used electronics to resist your electronic warfare!", structureOwner.GetName(), structure.GetTypeName()), true, player.GetID(), structureOwner.GetID()));
                        }
                        else
                        {
        ...
        if(player != null)
        {
            ...
            switch(structureType)
            {
                ...
                case ORE_MINE:
                {
                    ...
                    {
                        if(ValidateConstructionRequest(player, geoPosition, structureType.toString(), Costs))
                        {
    ...
    public boolean ConstructStructureFromBlueprint(int lPlayerID, int lBlueprintID)
    {
        ...
        if(player != null && blueprint != null)
        {
            ...
            {
                if(ValidateConstructionRequest(player, blueprint.GetPosition().GetCopy(), structureType.toString(), Costs))
                {
                    return AddStructure(lPlayerID, blueprint.GetPosition().GetCopy(), structureType, resourceType);
                }
            }
            else if(ValidateConstructionRequest(player, blueprint.GetPosition().GetCopy(), structureType.toString(), Costs))
            {
                return AddStructure(lPlayerID, blueprint.GetPosition().GetCopy(), structureType, resourceType);
            }
        }
                                ...
                                if(systemType == SystemType.STORED_AIRCRAFT_MISSILES)
                                {
                                    ...
                                    if((homebase instanceof Airbase && ((Airbase)homebase).GetOnline()) || homebase instanceof Ship)
                                    {
                                        ...
                                        if(type.GetAirLaunched() && type.GetPurchasable())
                                        {
                                            ...
                                            if(ProcessPlayerWeaponPurchase(lPlayerID, type.GetCosts(), null, null, true))
                                            {
                        ...
                        for(int lType : lLaunchableTypes)
                        {
                            ...
                            if(type != null)
                            {
                                ...
                                if(systemType == SystemType.STORED_AIRCRAFT_INTERCEPTORS && type.GetAirLaunched())
                                {
                                    ...
                                    if((homebase instanceof Airbase && ((Airbase)homebase).GetOnline()) || homebase instanceof Ship)
                                    {
                                        if(ProcessPlayerWeaponPurchase(lPlayerID, type.GetCosts(), null, null, false))
                                        {
                                            system.AddMissileToNextSlot(lSlotNo, lType, lPrepTime);
                                            EntityUpdated(homebase, false);
                                        }
    ...
    public boolean AirbaseCapacityUpgrade(int lPlayerID, int lAirbaseID)
    {
        ...
        if(airbase.GetAircraftSystem().GetSlotCount() < Defs.MAX_AIRBASE_CAPACITY)
        {
            ...
            if(player.GetCargoSystem().ChargeQuantity(typeCost, GetAircraftSlotUpgradeCost(airbase.GetAircraftSystem().GetSlotCount(), config.GetAirbaseBaseCapacity())))
            {
    ...
    public boolean TransferPlayerResources(int lPlayerID, EntityPointer pointerTo, int lTypeID, int lQuantity, boolean bLoadAsCargo)
    {
        //If entityTo is not null, we are transferring directly to an entity.
        Player player = GetPlayer(lPlayerID);
        MapEntity entityTo = pointerTo.GetMapEntity(this);
        
        if(player != null && player.Functioning())
        {
            if(entityTo != null)
            {
                Resource resource = player.GetCargoSystem().RemoveResource(ResourceType.values()[lTypeID], lQuantity);

                if(resource != null) //If resource is null, then the truck did not contain this type of resource.
                {
                    if(bLoadAsCargo && resource.GetQuantity() > 0 && entityTo instanceof HaulerInterface)
                    {
                        CargoSystem receiverSystem = ((HaulerInterface)entityTo).GetCargoSystem();
                    ...
                    {
                        //There is leftover. Add it back to the truck's cargo system. 
                        player.GetCargoSystem().AddResource(resource.GetResourceType(), resource.GetQuantity());
                    }

                    EntityUpdated(player, true);
                    EntityUpdated(entityTo, true);
                    return true;
                }
            }
        }
            ...
            if(player.Functioning() && !InBattle(player))
            {
                ...
                if(system != null)
                {
                    if(system.GetSlotHasMissile(lSlotIndex))
                    {
                        ...
                        ProcessPlayerIncome(player, String.format("sale of %s missile", type.GetName()), GetSaleValue(type.GetCosts()), false);
            ...
            if(player.Functioning() && !InBattle(player))
            {
                ...
                if(system != null)
                {
                    if(system.GetSlotHasMissile(lSlotIndex))
                    {
                        ...
                        system.UnloadSlot(lSlotIndex);
                        ProcessPlayerIncome(player, String.format("sale of %s interceptor", type.GetName()), GetSaleValue(type.GetCosts()), false);
            ...
            if(player.Functioning() && !InBattle(player))
            {
                ...
                if(system != null)
                {
                    if(system.GetSlotHasMissile(lSlotIndex))
                    {
                        ...
                        {
                            system.UnloadSlot(lSlotIndex);
                            ProcessPlayerIncome(player, String.format("sale of %s", type.GetName()), GetSaleValue(type.GetCosts()), false);
            ...
            if(bOnline)
            {
                if(player.GetWealth() >= Defs.ONLINE_MAINTENANCE_COST)
                {
                    ...
                    if(player.SubtractWealth(Defs.ONLINE_MAINTENANCE_COST))
                    {
                        ...
                        if(player.GetCargoSystem().ChargeQuantities(Defs.BOOSTER_TYPES_STRUCTURE_BOOT))
                        {
                            lBootTime *= Defs.BOOT_UP_BONUS_MULTIPLIER;
                            CreateEventForPlayer(new LaunchEvent(String.format("Your %s used %d electricity to boost boot-up time.", structure.GetTypeName(), Defs.BOOSTER_TYPES_STRUCTURE_BOOT.get(ResourceType.ELECTRICITY)), SoundEffect.BOOSTED_BOOT), lPlayerID);
                        }
    ...
    public boolean GiveWealth(int lAdminID, int lReceiverID, int lAmount, ResourceType type)
    {
        ...
        if(admin.GetIsAnAdmin())
        {
            ...
            {
                CargoSystem system = receiver.GetCargoSystem();
                
                if(system != null)
                {
                    system.AddResource(type, lAmount);
    ...
    public boolean RepairEntity(int lPlayerID, EntityPointer pointer)
    {
        ...
        if(player != null && player.Functioning() && !InBattle(player))
        {
            if(entity instanceof Structure)
            {
                ...
                {
                    if(player.GetCargoSystem().ChargeQuantities(GetRepairCost(structure)))
                    {
            ...
            else if(entity instanceof Shipyard)
            {
                ...
                {
                    if(player.GetCargoSystem().ChargeQuantities(GetRepairCost(shipyard)))
                    {
            ...
            else if(entity instanceof Infantry)
            {
                ...
                {
                    if(ProcessPlayerPurchase(lPlayerID, Map.ofEntries(entry(ResourceType.MEDICINE, (long)(infantry.GetHPDeficit() * Defs.MEDICINE_PER_HP_INFANTRY_KG))), null, player.GetCargoSystem(), true, PurchaseType.OFFENSIVE))
                    {
            ...
            else if(entity instanceof Tank)
            {
                ...
                {
                    if(ProcessPlayerPurchase(lPlayerID, Map.ofEntries(entry(ResourceType.STEEL, (long)(tank.GetHPDeficit() * Defs.STEEL_PER_HP_REPAIR_KG))), null, player.GetCargoSystem(), true, PurchaseType.OFFENSIVE))
                    {
                        tank.FullyRepair();
            ...
            else if(entity instanceof CargoTruck)
            {
                ...
                {
                    if(ProcessPlayerPurchase(lPlayerID, Map.ofEntries(entry(ResourceType.STEEL, (long)(truck.GetHPDeficit() * Defs.STEEL_PER_HP_REPAIR_KG))), null, player.GetCargoSystem(), true, PurchaseType.ECONOMIC))
                    {
                        truck.FullyRepair();
            ...
            else if(entity instanceof Ship)
            {
                ...
                {
                    if(ProcessPlayerPurchase(lPlayerID, Map.ofEntries(entry(ResourceType.STEEL, (long)(ship.GetHPDeficit() * Defs.STEEL_PER_HP_REPAIR_KG))), null, player.GetCargoSystem(), false, PurchaseType.OFFENSIVE))
                    {
                        ship.FullyRepair();
            ...
            else if(entity instanceof Submarine)
            {
                ...
                {
                    if(ProcessPlayerPurchase(lPlayerID, Map.ofEntries(entry(ResourceType.STEEL, (long)(submarine.GetHPDeficit() * Defs.STEEL_PER_HP_REPAIR_KG))), null, player.GetCargoSystem(), true, PurchaseType.OFFENSIVE))
                    {
            ...
            else if(entity instanceof Rubble)
            {
                ...
                if(!rubble.GetRemove() && rubble.GetOwnerID() == lPlayerID && player.Functioning())
                {
                    ...
                    if(ProcessPlayerPurchase(lPlayerID, GetRepairCost(rubble), null, player.GetCargoSystem(), false, PurchaseType.DEFENSIVE))
                    {
    ...
    {
        Player player = GetPlayer(lPlayerID);
        MapEntity mapAirbase = pointerAirbase.GetMapEntity(this);
        AirplaneInterface storedAircraft = (AirplaneInterface)pointerAircraft.GetEntity(game);
        
        if(player != null && storedAircraft != null && mapAirbase != null)
        {
            CargoSystem system = player.GetCargoSystem();
            
            if(mapAirbase instanceof Airbase)
            {
                Airbase airbase = (Airbase)mapAirbase;
                
                if(airbase.GetOnline())
                {
                    //TODO: Record for player.
                    if(system.ChargeQuantity(ResourceType.FUEL, (int)storedAircraft.GetFuelDeficit()))
                    {
                        storedAircraft.Refuel();
                    }
                } 
            }
            else if(mapAirbase instanceof Ship)
            {
                //TODO: Record for player.
                if(system.ChargeQuantity(ResourceType.FUEL, (int)storedAircraft.GetFuelDeficit()))
                {
    ...
    public void LandAircraft(EntityPointer pointerAircraft, AircraftSystem system)
    {
        ...
        if(entity instanceof Airplane)
        {
            ...
            if(owner != null)
            {
                ...
                owner.GetCargoSystem().ChargeQuantity(ResourceType.WEALTH, oAmountCharged);
    ...
    public void LaunchDebugMissiles(int lMissileCount)
    {
        ...
        {
            if(type.GetICBM() || type.GetMissileSpeed() < 10000)
                continue;
        ...
        {
            Player randomPlayer1 = (Player)GetPlayers().toArray()[random.nextInt(0, GetPlayers().toArray().length)];
            Player randomPlayer2 = (Player)GetPlayers().toArray()[random.nextInt(0, GetPlayers().toArray().length)];
            
            MissileType type = (MissileType)Types.toArray()[random.nextInt(0, Types.toArray().length)];
            
            if(type != null && randomPlayer1 != null && randomPlayer2 != null && randomPlayer1.GetID() != randomPlayer2.GetID())
            {
                GeoCoord geoLaunch = randomPlayer1.GetPosition().GetCopy();
                GeoCoord geoTarget = randomPlayer2.GetPosition().GetCopy();
                LaunchLog.ConsoleMessage("Launching missile...");
                CreateMissileLaunch(randomPlayer1, geoLaunch, type.GetID(), randomPlayer1.GetID(), geoTarget, null, false);
            } 
        }
    }
    ...
    public void ProcessPlayerIncome(Player payee, String strSource, Map<ResourceType, Long> Incomes, boolean bTaxable)
    {
        for(Entry<ResourceType, Long> income : Incomes.entrySet())
        {
            ...
            {
                payee.GetCargoSystem().AddResource(income.getKey(), income.getValue());
            }
        }
    ...
    public boolean RefuelEntity(int lPlayerID, EntityPointer pointer) 
    {
        ...
        if(entity instanceof StoredAirplane)
        {
            ...
            if(airplane.GetOwnerID() == lPlayerID && player.Functioning())
            {
                ...
                if(ProcessPlayerPurchase(lPlayerID, Map.ofEntries(entry(ResourceType.FUEL, (long)lFuelCost)), null, player.GetCargoSystem(), true, PurchaseType.OFFENSIVE))
                {
                    airplane.Refuel();
        ...
        else if(entity instanceof NavalVessel)
        {
            ...
            if(vessel.GetOwnerID() == lPlayerID && player.Functioning())
            {
                ...
                if(ProcessPlayerPurchase(lPlayerID, Map.ofEntries(entry(ResourceType.OIL, (long)lOilCost)), null, player.GetCargoSystem(), true, PurchaseType.OFFENSIVE))
                {
                    vessel.Refuel();
